...
from _constants import (
    ...
    URL_FILING,
    URL_SUBMISSIONS, FORM_FULL_SUBMISSION_FILENAME, HOST_WWW_SEC, HOST_DATA_SEC, URL_CIK_MAPPING,
)
...
class DownloadFormManager:
    def __init__(self, user_agent: str):
        self.user_agent = user_agent

    def get_forms_save_location(self, download_metadata: FormsDownloadMetadata,
    ...
    def save_form_document(self,
        ...
        save_path = self.get_forms_save_location(download_metadata, accession_number, save_filename)
    ...
    def aggregate_forms_to_download(self,
        ...
        while fetched_count < download_metadata.limit and filings_available:
            resp_json = self._rate_limited_get(submissions_uri, host=HOST_DATA_SEC).json()
            ...
            for acc_num, form, doc, f_date in zip(
                ...
                filings_to_download.append(
                    self.get_to_download(download_metadata.cik, acc_num, doc)
                )
    ...
    def get_to_download(self, cik: str, acc_num: str, doc: str) -> FormsToDownload:
        cik = cik.strip("0")
    ...
    def fetch_and_save_filings(self, download_metadata: FormsDownloadMetadata) -> int:
        ...
        for td in to_download:
            ...
            self.save_form_document(raw_filing, download_metadata, td.accession_number, FORM_FULL_SUBMISSION_FILENAME)
            ...
            if download_metadata.download_details:
                ...
                self.save_form_document(primary_doc, download_metadata, td.accession_number, primary_doc_filename)
