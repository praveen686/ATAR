...
class Binance_Orders_Handler(Binance_Orderbook_Handler):
    ...
    def add_trailing_stop_loss_to_order(BOT, position_order=None, callbackRate=None, activationPrice=None):

        BOT.log.info(
            f"placing trailing stop loss for order {position_order['id']} with callbackRate {callbackRate} and activationPrice {activationPrice}")
        # Set a stop loss and take profit orders to protect our position and to obide by our risk management rules.
        side = position_order.entry
        # order_price = order_json['price']
        ...
        trailingStopOrder = BOT.exchange.create_order(symbol=position_order.instrument_id, type='TRAILING_STOP_MARKET',
                                                      side=side,
                                                      amount=position_order.quantity, params=trailing_stop_params)
