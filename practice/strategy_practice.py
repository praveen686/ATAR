...
class OrderBookImbalance(Strategy):
    ...
    def check_trigger(self):
        ...
        self.log.info(f"Bid_Size: {bid_size} Ask_Size: {ask_size}")
        ...
        if larger > self.trigger_min_size and ratio < self.trigger_imbalance_ratio:
            if len(self.cache.orders_inflight(strategy_id=self.id)) > 0:
                ...
                if ask_size > self.max_trade_size:
                    ask_size = self.max_trade_size
                ...
                if bid_size > self.max_trade_size:
                    bid_size = self.max_trade_size
                order = self.order_factory.limit(
                    instrument_id=self.instrument.id,
        ...
        # # When order filled then set  upt exit condition
        # elif not self.portfolio.is_flat(self.instrument_id):
        #     self.close_all_positions(self.instrument_id)
        #     self.log.info(f'Unrealized PnL: {self.portfolio.unrealized_pnl(self.instrument_id)}')
        #     #
        #     # net_position = self.portfolio.net_position(self.instrument_id)
        #     # order = self.order_factory.market(
        #     #     instrument_id=self.instrument.id,
        #     #     order_side=OrderSide.BUY if net_position < 0 else OrderSide.SELL,
        #     #     quantity=self.instrument.make_qty(
        #     #         # Size of position
        #     #         abs(self.portfolio.net_position(self.instrument_id))
        #     #     ),
        #     # )
        #     # self.submit_order(order)
